...
import { GATEWAY_URL, getGatewayHostForLog } from "@/lib/gateway";
...
export async function POST(request: Request) {
  ...
  if (!res.ok) {
    ...
    const payload: Record<string, unknown> = {
      ...
      code: formatted.code,
      gatewayCode: typeof data.code === "string" ? data.code : undefined,
      fieldErrors: formatted.fieldErrors,
    };
    if (process.env.NODE_ENV !== "production") {
      payload.gatewayHost = getGatewayHostForLog();
    }
