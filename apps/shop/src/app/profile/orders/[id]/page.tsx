...
export default function ProfileOrderDetailPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <Link
        href="/profile/orders"
        className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-900 px-2"
      >
        <CaretLeft size={18} weight="bold" />
        <span className="font-bold text-[14px]">Back to orders</span>
      </Link>
      ...
      {o ? (
        <>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-2">
            ...
            <span className="px-4 py-2 bg-[#2D4D31]/10 text-[#2D4D31] rounded-xl text-[13px] font-black uppercase tracking-wider w-fit">
              {statusLabel}
            </span>
          </div>
